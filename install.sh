#!/bin/sh
if [ -e ezserver.standard_bit64.tar ]; then
rm -f ezserver.standard_bit64.tar
fi
read  -p "Please enter installation password? " dpass
if test -z $dpass; then
exit 0
fi
standard_url='http://www.ezhometech.com/download_standard_'$dpass'/ezserver.standard_bit64.tar'
wget -O ezserver.standard_bit64.tar $standard_url
if [ -s ezserver.standard_bit64.tar ]; then
	echo "ezserver standard version downloaded..."
	if [ -e ezserver_standard ]; then
		backupfilename="ezserver_standard$(date +%Y%m%d_%s)"
		read  -p "Backup current ezserver_standard folder(?(y/n) " yn
		if [ "$yn" != "Y" ] && [ "$yn" != "y" ]; then
			rm -rf ezserver_standard
			echo "Remove ezserver_standard folder"
		else
			mv ezserver_standard $backupfilename
			echo "Backup ezserver_standard folder to "$backupfilename		
		fi
	fi
	tar xfvz ezserver.standard_bit64.tar
	rm ezserver.standard_bit64.tar
else
	echo "Password Error..."
	exit 0
fi
cd ezserver_standard
chmod 777 *.*
chmod 777 *
